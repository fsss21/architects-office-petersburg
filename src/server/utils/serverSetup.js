...
/**
 * Конфигурация сервера — проект «Классический Петербург» (architects-office-petersburg)
...
class ServerSetup {
  ...
  async initializeDataDir() {
    try {
      ...
      if (this.progressPointsFileFallback || this.progressPointsFile) {
...
<head>
  ...
  <title>Запуск приложения — Классический Петербург</title>
  <style>
...
<body>
  ...
  <ol>
    ...
    <li>Сборка Vite (Классический Петербург) попадает в папку <code>build</code>. Запускайте сервер из корня проекта — статика будет браться из <code>build/</code>.</li>
